...
export const useEvento = (idEvento = null) => {
    ...
    const cargarEvento = async (id) => {
        try {
            ...
            if (evento.id_lugar && lugares.length) {
                ...
                const posibleUbicacion = lugar?.id_ubicacion ?? lugar?.ubicacion_id ?? lugar?.ubicacion?.id;
                if (posibleUbicacion) setUbicacionSeleccionada(posibleUbicacion);
            }
            const actsRes = await obtenerActividadesEvento(id);
            const acts = Array.isArray(actsRes.data) ? actsRes.data : [actsRes.data];
            setActividades(
                acts.length
                    ? acts.map(a => ({
                        id: a.id,
                        ...
                        fecha_actividad: a.fecha_actividad ? new Date(a.fecha_actividad).toISOString().split('T')[0] : '',
    ...
    const handleInputChange = (campo, valor) => {
        ...
        if (campo === 'id_lugar' || campo === 'cupos') {
            const idLugar = campo === 'id_lugar' ? valor : formData.id_lugar;
            const cupos = campo === 'cupos' ? valor : formData.cupos;
            if (idLugar && cupos && (formData.modalidad === 'Presencial' || formData.modalidad === 'Híbrido')) {
                validarCuposContraCapacidad(idLugar, cupos);
            }
        }
        if (campo === 'modalidad') {
            if (valor === 'Virtual') setFormData(prev => ({ ...prev, id_lugar: '' }));
            else if (valor === 'Presencial') setFormData(prev => ({ ...prev, url_virtual: '' }));
        }
    };
